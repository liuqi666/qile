                    ...
                    if (empty($cinfo['nocommission'])) {
                        if ($cinfo['hascommission'] == 1) {
                            $cinfo['commission1'] = array(
                                'default' => $set['level'] >= 1 ? ($cinfo['commission1_rate'] > 0 ? round($cinfo['commission1_rate'] * ($price - $cinfo['costprice']) / 100, 2) . "" : round($cinfo['commission1_pay'] * $cinfo['total'], 2)) : 0
                            );
                            $cinfo['commission2'] = array(
                                'default' => $set['level'] >= 2 ? ($cinfo['commission2_rate'] > 0 ? round($cinfo['commission2_rate'] * ($price - $cinfo['costprice']) / 100, 2) . "" : round($cinfo['commission2_pay'] * $cinfo['total'], 2)) : 0
                            );
                            $cinfo['commission3'] = array(
                                'default' => $set['level'] >= 3 ? ($cinfo['commission3_rate'] > 0 ? round($cinfo['commission3_rate'] * ($price - $cinfo['costprice']) / 100, 2) . "" : round($cinfo['commission3_pay'] * $cinfo['total'], 2)) : 0
                            );
                        } else {
                            $cinfo['commission1'] = array(
                                'default' => $set['level'] >= 1 ? round($set['commission1'] * ($price - $cinfo['costprice']) / 100, 2) . "" : 0
                            );
                            $cinfo['commission2'] = array(
                                'default' => $set['level'] >= 2 ? round($set['commission2'] * ($price - $cinfo['costprice']) / 100, 2) . "" : 0
                            );
                            $cinfo['commission3'] = array(
                                'default' => $set['level'] >= 3 ? round($set['commission3'] * ($price - $cinfo['costprice']) / 100, 2) . "" : 0
                            );
                            foreach ($levels as $level) {
                                $cinfo['commission1']['level' . $level['id']] = $set['level'] >= 1 ? round($level['commission1'] * ($price - $cinfo['costprice']) / 100, 2) . "" : 0;
                                $cinfo['commission2']['level' . $level['id']] = $set['level'] >= 2 ? round($level['commission2'] * ($price - $cinfo['costprice']) / 100, 2) . "" : 0;
                                $cinfo['commission3']['level' . $level['id']] = $set['level'] >= 3 ? round($level['commission3'] * ($price - $cinfo['costprice']) / 100, 2) . "" : 0;
                            }
                        }
                    } else {
                        $cinfo['commission1'] = array(
